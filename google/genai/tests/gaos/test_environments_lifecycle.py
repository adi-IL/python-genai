...
def test_python_environments_files_get_and_download(monkeypatch):
  ...
  try:
    ...
    # Test sync files.get with pagination and recursive options
    files_res_paginated = client.environments.files.get(
        environment="env_123",
        path="src",
        page_size=10,
    ...
    # Test sync files.list
    files_res_list = client.environments.files.list(
        environment="env_123",
        path="src",
        page_size=10,
        page_token="token_start",
    ...
    assert len(files_res_list.files) == 1
...
async def test_python_environments_async_files_get_and_download(monkeypatch):
  ...
  try:
    ...
    # Test async files.get with pagination and recursive options
    files_res_paginated = await client.aio.environments.files.get(
        environment="env_123",
        path="src",
        page_size=10,
        page_token="token_start",
    ...
    # Test async files.list
    files_res_list = await client.aio.environments.files.list(
        environment="env_123",
        path="src",
        page_size=10,
        page_token="token_start",
...
def test_python_environments_files_wrapper_error_handling():
  ...
  with pytest.raises(
      AttributeError,
      match="environments.files.get is not available on this client.",
  ):
    wrapper.get(environment="env_123", path="src/main.py")
  ...
  with pytest.raises(
    ...
    asyncio.run(async_wrapper.get(environment="env_123", path="src/main.py"))
